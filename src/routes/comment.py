...
from src.schemas import CommentCreateSchema, ReturnMessageResponseSchema, CommentDb, CommentShowAllSchema, \
...
async def create_comment_for_image(
    ...
    new_comment = await repository_comment.create_comment(comment, current_user.id, image_id, db)
    return new_comment
...
async def delete_comment_for_image(
        ...
        db: AsyncSession = Depends(get_db),
        current_user: User = Depends(auth_service.get_current_user)):
    deleted_comment = await repository_comment.delete_comment(comment_id, current_user, db)
